...
if currency_columns:  # Check if currency_columns is not empty
    ...
    curr1 = st.selectbox('', currency_columns, index=currency_columns.index(default_currency1))
    st.markdown("<div style='font-size: 20px; color: white;'>Select the Currency2</div>", unsafe_allow_html=True)
    curr2= st.selectbox('', currency_columns, index=currency_columns.index(default_currency2))
else:
...
if update_button and curr1!="" and curr2!="":
    from python.NT import diffcurr  #importing function for finding difference in currencies
    json_data = diffcurr(curr1, curr2, start_date, end_date) #data obtained from function in json format
    ...
    max_value = resampled_df['Values'].max() #Max value
    st.markdown(f'<p class="big-font"><u><b>Maximum Value</u></b>: {max_value}</p>', unsafe_allow_html=True)
    min_value = resampled_df['Values'].min() #Min value
    st.markdown(f'<p class="big-font"><u><b>Minimum Value</u></b>: {min_value}</p>', unsafe_allow_html=True)